...
def plotBestConfigurations(files, sta_files, names, topoName, topoFile, rewardFiles):
	...
	for i in range(len(bestConfigs)):
		...
		plot2dTopology(topoFile, topoName, receivingPower, configuration=lx)
...
topos = ["T7", "C6o", "T12"]
tests = ["DEFAULT_UNI_ADHOC", "EGREED_UNI_ADHOC", "TNORM_UNI_ADHOC", "TNORM_HGMT_ADHOC", "TGNORM_HGMT_ADHOC"]
...
for topo in topos:
	for testDuration in testDurations:
		...
		# names = ["BEST 0.7"]
		names = ["DEFAULT", "EGREEDY", "TS", "GM-TS", "OURS"]
